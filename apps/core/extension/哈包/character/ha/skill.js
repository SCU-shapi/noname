...
const skill = {
	...
	youxian: {
		...
		filter(event, player) {
			return event.player !== player && event.player.isIn() && !player.storage._youxian_used && player.countCards("h") > 0;
		},
		async cost(event, trigger, player) {
			const result = await player.chooseBool("游仙：是否让" + get.translation(trigger.player) + "声明一张牌？");
			event.result = { bool: result.bool };
		},
		async content(event, trigger, player) {
			player.storage._youxian_used = true;
			...
			const chooseResult = await target.chooseButton(["游仙：请声明一张牌", [list, "vcard"]], true)
				.forResult();
			if (!chooseResult.bool) return;
			const cardName = chooseResult.links[0][2];
			await player.draw(1);
			if (player.hasCard(c => c.name === cardName, "h")) {
				const giveResult = await player.chooseToGive(target, "h", true, "游仙：请选择一张" + get.translation(cardName) + "交给" + get.translation(target))
					.set("filterCard", card => card.name === cardName)
					.set("ai", card => get.value(card) < 5 ? 1 : 0)
					.forResult();
				if (giveResult.bool && target.hasCard(c => c === giveResult.cards[0], "h")) {
					await target.useCard(giveResult.cards[0], null, false);
				}
			} else {
		...
		subSkill: {
			reset: {
				...
				trigger: { global: "roundStart" },
				content(event, trigger, player) {
					delete player.storage._youxian_used;
				},
			},
		},
		ai: {
	...
	chujiang: {
		enable: "phaseUse",
		filter(event, player) {
			return !player.isTurnedOver() && player.countCards("h") > 0;
		},
		async content(event, trigger, player) {
			const result = await player.chooseCard("h", true, "厨将：选择一张手牌，将其在本回合结束前视为桃/酒")
				.set("ai", card => 6 - get.value(card))
				.forResult();
			...
			card._chujiang_mark = true;
			player.addTempSkill("chujiang_effect", "phaseAfter");
		},
		group: "chujiang_effect",
		subSkill: {
			effect: {
				mod: {
					cardname(card, player) {
						if (card._chujiang_mark) return "tao";
					},
				},
				onremove(player) {
					const cards = player.getCards("h");
					for (const c of cards) {
						delete c._chujiang_mark;
					}
				},
			},
		},
	...
	yinjun: {
		...
		async content(event, trigger, player) {
			const source = trigger.player;
			player.logSkill("yinjun", source);
			if (source.countCards("he") > 0) {
				const giveResult = await source.chooseToGive(player, "he", true, "隐君：请交给" + get.translation(player) + "一张牌，否则此杀无效");
				if (!giveResult.bool) {
					trigger.getParent().targets.remove(player);
					game.log(trigger.card, "对", player, "无效");
				}
	...
	jibai: {
		...
		filter(event, player) {
			return player.hp < 2 && !player.storage._jibai_used;
		},
		async content(event, trigger, player) {
			player.awakenSkill("jibai");
			player.storage._jibai_used = true;
			await player.turnOver();
			player.maxHp--;
			player.update();
			const orderedPlayers = game.filterPlayer(p => p.isIn()).sortBySeat();
			for (const p of orderedPlayers) {
				if (p === player) continue;
				if (p.hp > player.hp || p.countCards("h") > player.countCards("h")) {
					await p.chooseToDiscard("he", 2, true, "基摆：请弃置两张牌");
				} else {
					p.addSkills("jibao");
				}
			}
	...
	jibao: {
		...
		async content(event, trigger, player) {
			...
			player.logSkill("jibao", source);
			if (player.canUse({ name: "sha", isCard: true }, source, false)) {
				const next = player.useCard({ name: "sha", isCard: true }, source, false);
				player.addTempSkill("jibao_track", { player: "useCardAfter" });
				player.storage._jibao_target = source;
				await next;
				if (player.storage._jibao_attacked) {
					if (player.canUse({ name: "sha", isCard: true }, source, false)) {
						await player.useCard({ name: "sha", isCard: true }, source, false);
					}
				}
				player.removeSkills("jibao_track");
				delete player.storage._jibao_target;
				delete player.storage._jibao_attacked;
			}
		},
		group: "jibao_track",
		subSkill: {
			track: {
				...
				forced: true,
				filter(event, player) {
					return event.player === player.storage._jibao_target && event.card && event.card.name === "shan";
				},
				content(event, trigger, player) {
					player.storage._jibao_attacked = true;
				},
			},
		},
		ai: {
	...
	yinqing: {
		...
		async content(event, trigger, player) {
			...
			if (card) {
				player.addToExpansion(card, player, "give");
				const color = get.color(card);
				player.storage.yinqing_zone.push(card);
				if (color === "red") {
					player.addSkills("lianji");
					player.removeSkills("danji");
				} else {
					player.addSkills("danji");
					player.removeSkills("lianji");
				}
			}
			const zone = player.storage.yinqing_zone;
			const colors = zone.map(c => get.color(c));
			const hasDuplicates = colors.some((c, i) => colors.indexOf(c) !== i);
			if (hasDuplicates) {
				const chooseResult = await player.chooseButton(["阴晴：请选择一张牌收回", zone], true).forResult();
				if (chooseResult.bool) {
					const chosen = chooseResult.links[0];
					await player.gain(chosen, "draw");
				}
				player.loseToDiscardpile(zone);
				player.storage.yinqing_zone = [];
			}
		},
		mark: true,
		intro: {
			...
			content(storage, player) {
				return "演舞区共有" + (player.storage.yinqing_zone || []).length + "张牌";
			},
		},
		onremove(player) {
			const cards = player.getExpansions("yinqing");
			if (cards.length) player.loseToDiscardpile(cards);
			delete player.storage.yinqing_zone;
			player.removeSkills("lianji");
			player.removeSkills("danji");
		},
		ai: {
	...
	lianji: {
		...
		filter(event, player) {
			...
			const zone = player.storage.yinqing_zone || [];
			return zone.some(c => get.suit(c) === get.suit(event.card));
		},
		async content(event, trigger, player) {
			const user = trigger.player;
			player.logSkill("lianji", user);
			await player.draw(1);
			await user.draw(1);
		},
		mod: {
			targetEnabled(card, player, target) {
				...
				const zone = target.storage.yinqing_zone || [];
				if (zone.some(c => get.suit(c) === get.suit(card))) return false;
			},
		},
		ai: {
			order: 6,
	...
	danji: {
		...
		filter(event, player) {
			...
			return player.hasCard(c => {
				return zone.some(zc => get.suit(zc) === get.suit(c)) && !player.canUse(c, null, false);
			}, "h");
		},
		filterCard(card, player) {
			const zone = player.storage.yinqing_zone || [];
			return zone.some(zc => get.suit(zc) === get.suit(card)) && !player.canUse(card, null, false);
		},
		async content(event, trigger, player) {
	...
	xiaoshi: {
		...
		locked: true,
		init(player) {
			player.addSkills("liudao_xiayi");
			player.addSkills("liudao_zhengpai");
			player.addSkills("liudao_shuangbo");
			player.addSkills("liudao_qinwu");
			player.addSkills("liudao_bubai");
			player.addSkills("liudao_zongheng");
			player.storage.liudao_skills = ["liudao_xiayi", "liudao_zhengpai", "liudao_shuangbo", "liudao_qinwu", "liudao_bubai", "liudao_zongheng"];
			player.storage.liudao_disabled = [];
		},
		trigger: { player: "changeHp" },
		filter(event, player) {
		...
		async content(event, trigger, player) {
			...
			if (removed) {
				player.removeSkills(removed);
				if (!player.storage.liudao_removed) player.storage.liudao_removed = [];
				player.storage.liudao_removed.push(removed);
			}
		},
		mod: {
			maxHp(player, num) { return num; },
			recover(player, num) { return 0; },
		},
		ai: {
			order: 9,
			result: { player: 1 },
	...
	lunhui: {
		...
		filterCard(card, player) {
			...
			return typeof num === "number" && num >= 1 && num <= (player.storage.liudao_skills.length);
		},
		position: "h",
		async content(event, trigger, player) {
			const card = event.cards[0];
			const x = get.number(card);
			const skillName = player.storage.liudao_skills[x - 1];
			if (!skillName) return;
			player.logSkill("lunhui");
			await player.useSkill(skillName);
			player.removeSkills(skillName);
			const idx = player.storage.liudao_skills.indexOf(skillName);
			if (idx >= 0) player.storage.liudao_skills.splice(idx, 1);
			if (!player.storage.liudao_disabled) player.storage.liudao_disabled = [];
			player.storage.liudao_disabled.push(skillName);
			player.storage.lunhui_points = (player.storage.lunhui_points || 0) + x;
			if (player.storage.lunhui_points % player.hp === 0) {
				player.storage.lunhui_points = 0;
				const disabled = player.storage.liudao_disabled || [];
				for (const s of disabled) {
					player.addSkills(s);
					player.storage.liudao_skills.push(s);
				}
				player.storage.liudao_disabled = [];
			}
		},
	...
	liudao_shuangbo: {
		...
		async content(event, trigger, player) {
			...
			const targets = game.filterPlayer(t => t !== player && t.isIn() && player.canUse({ name: "sha", isCard: true }, t, false));
			if (targets.length) {
				const result = await player.chooseTarget("双搏：请选择杀的目标", (card, p, target) => player.canUse({ name: "sha", isCard: true }, target, false))
					.set("ai", target => get.effect(target, { name: "sha" }, player, player))
					.forResult();
				if (result.bool && result.targets && result.targets.length) {
					await player.useCard({ name: "sha", isCard: true }, result.targets, false);
				}
			}
		},
		ai: {
	...
	liudao_qinwu: {
		...
		async cost(event, trigger, player) {
			const result = await player.chooseToDiscard("h", true, "勤务：是否弃置一张手牌，令" + get.translation(trigger.player) + "额外使用一次此牌？");
			event.result = { bool: result.bool };
		},
		async content(event, trigger, player) {
			const user = trigger.player;
			const cardName = get.name(trigger.card);
			player.logSkill("liudao_qinwu", user);
			const targets = game.filterPlayer(t => user.canUse({ name: cardName, isCard: true }, t, false));
			if (targets.length) {
				const result = await user.chooseTarget("勤务：请为" + get.translation(cardName) + "选择目标", (card, p, t) => user.canUse({ name: cardName, isCard: true }, t, false))
					.set("ai", t => get.effect(t, { name: cardName }, user, user))
					.forResult();
				if (result.bool && result.targets && result.targets.length) {
					await user.useCard({ name: cardName, isCard: true }, result.targets, false);
				}
			}
		},
		group: "liudao_qinwu_self",
		subSkill: {
			self: {
				trigger: { player: "useCardAfter" },
				filter(event, player) {
					return event.card && event.card.name !== "liudao";
				},
				async content(event, trigger, player) {
					const result = await player.chooseBool("勤务：是否让其他角色自愿弃置一张手牌，使你额外使用一次此牌？");
					if (!result.bool) return;
					const others = game.filterPlayer(p => p !== player && p.isIn() && p.countCards("h") > 0).sortBySeat();
					for (const other of others) {
						const boolResult = await other.chooseBool("勤务：是否自愿弃置一张手牌，令" + get.translation(player) + "额外使用一次" + get.translation(trigger.card) + "？");
						if (boolResult.bool) {
							const discardResult = await other.chooseToDiscard("h", true, "勤务：请弃置一张手牌");
							if (discardResult.bool) {
								const cardName = get.name(trigger.card);
								const targets = game.filterPlayer(t => player.canUse({ name: cardName, isCard: true }, t, false));
								if (targets.length) {
									const tResult = await player.chooseTarget("勤务：请为" + get.translation(cardName) + "选择目标", (card, p, t) => player.canUse({ name: cardName, isCard: true }, t, false))
										.set("ai", t => get.effect(t, { name: cardName }, player, player))
										.forResult();
									if (tResult.bool && tResult.targets && tResult.targets.length) {
										await player.useCard({ name: cardName, isCard: true }, tResult.targets, false);
									}
								}
							}
							break;
	...
	liudao_bubai: {
		trigger: { player: "useCard" },
		filter(event, player) {
			return event.card && event.card.name === "sha" && player.countCards("he") >= 2;
		},
		async cost(event, trigger, player) {
			const result = await player.chooseToDiscard("he", 2, "不败：是否弃置两张牌，令此杀不可响应并获得目标一张牌？");
			event.result = { bool: result.bool };
		},
		async content(event, trigger, player) {
			player.logSkill("liudao_bubai");

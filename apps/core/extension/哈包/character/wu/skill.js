...
const skill = {
	...
	gefang: {
		...
		limited: true,
		filter(event, player) {
			return player.isZhu && !player.storage._gefang_used;
		},
		async cost(event, trigger, player) {
			const result = await player.chooseBool("革放：是否把摸牌阶段摸牌规则改为摸" + Math.min(player.hp, 5) + "张牌？");
			event.result = { bool: result.bool };
		},
		async content(event, trigger, player) {
			player.storage._gefang_used = true;
			player.logSkill("gefang");
			const hp = Math.min(player.hp, 5);
			player.addTempSkill("gefang_draw");
			player.storage._gefang_draw = hp;
		},
		group: "gefang_draw",
		subSkill: {
			draw: {
				...
				filter(event, player) {
					return player.storage._gefang_draw > 0;
				},
				async content(event, trigger, player) {
					trigger.num = player.storage._gefang_draw;
					delete player.storage._gefang_draw;
					player.removeSkills("gefang_draw");
				},
			},
		},
		ai: {
